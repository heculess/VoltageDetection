...
static const char *TAG = "EspClass";
...
std::string EspClass::getSketchMD5()
{
    ...
    uint32_t offset = 0;
    if(!buf.get()) {
        ESP_LOGE(TAG, "Not enough memory to allocate buffer");
    ...
    while( lengthLeft > 0) {
        ...
        if (!ESP.flashRead(running->address + offset, reinterpret_cast<uint32_t*>(buf.get()), (readBytes + 3) & ~3)) {
            ESP_LOGE(TAG, "Could not read buffer from flash");

            return std::string();
        }
        md5.update(buf.get(), readBytes);
